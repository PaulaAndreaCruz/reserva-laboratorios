...
app.post('/api/reservations', async (req, res) => {
  ...
  // Verificar horarios bloqueados por clase
  const toMins = t => { const [h,m] = t.split(':').map(Number); return h*60+m; };
  const overlaps = (s1,e1,s2,e2) => toMins(s1) < toMins(e2) && toMins(e1) > toMins(s2);
  const d = new Date(date + 'T12:00:00');
  const jsDay = d.getDay();
  const dayIdx = jsDay === 0 ? -1 : jsDay - 1;
  if (dayIdx >= 0 && dayIdx <= 4) {
    const blocks = await Schedule.find({ lab, day: dayIdx });
    const blockedBy = blocks.find(b => overlaps(start_time, end_time, b.start_time, b.end_time));
    if (blockedBy)
      return res.status(400).json({
        error: 'Horario bloqueado por clase de tecnología/ingeniería: ' + blockedBy.start_time + ' – ' + blockedBy.end_time + '. Elige otro horario.'
      });
  }
 
  // Verificar conflicto con otras reservas
  const existing = await Reservation.find({ lab, date });
  const conflict = existing.find(r =>
    toMins(start_time) < toMins(r.end_time) && toMins(end_time) > toMins(r.start_time)
  );
  if (conflict)
    return res.status(400).json({
      error: 'Este laboratorio ya tiene una reserva ese día de ' + conflict.start_time + ' a ' + conflict.end_time + ' (' + conflict.teacher + '). Elige otro horario.'
    });
